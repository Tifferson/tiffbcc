...
<head>
	...
	//	$.each(addresses, function(i, val){ geocode(i, val) })
		
	}
	
	function geocode(id, address){
		$.get('/proxy.php', {'proxy_url': 'http://rpc.geocoder.us/service/csv?address=' + address},
			function(data){
				row = data.split(',')
				glat_lang = new GLatLng(row[0], row[1])
				add_to_map(id, address, glat_lang)
			})
	 ...
	 geocoder.getLatLng(
	    ...
	    function(point) {
	      if (!point) {
			...
			add_to_map(id, address, point);
	      }
	    }
	  );
	}
	
	function add_to_map(id, address, point){
...
<body onload="initialize()" onunload="GUnload()">
	...
	<p><a href="javascript:submit_addresses()">Get Directions!</a></p>		
	</div>
	...
	<div id="locations_panel">
		<dl>
			...
			<!-- preg_replace('/\s[SB|NB|EB|WB]/', '',  -->
				<? $class = $i % 2 == 1 ? '' : 'alt'; ?>
				<? $address = clean_address($row->dgf_address) . ", " . $row->city . ", TX " . $row->zip ?>
				<dt class="di_<?=$row->id?> <?=$class?>"><a href="javascript:showAddress('none', '<?=$row->id?>', '<?= $address ?>', '<?= addslashes($row->dgf_name )?>')"><?= $row->dgf_name ?></a> 

...
class Leads extends OC_Controller{
  ...
  var $upload_config;
	...
	function follow_ups(){ 
		...
		// $this->db->where('is_lead', '1');
		// $this->db->where("ignore != '1'");
        // $this->db->where("id IN (SELECT location_id FROM follow_ups where done=0)",)
		// $this->db->order_by("name asc");


		// $data['follow_ups'] = $this->db->get('locations');
	
		$this->subnav_active = "follow_ups";
		// $this->simple_page('follow_ups', $data);
		$this->simple_page('follow_ups', '');	
